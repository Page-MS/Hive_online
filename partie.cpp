...
void Partie::commencerPartie(const QString& pseudo1, const QString& pseudo2, bool ia1, bool ia2) {
    // Initialisation des joueurs
    if (ia1 && ia2) {
        historique_etats[0].joueurs[0] = new IAJoueur(pseudo1.toStdString(), true);
        historique_etats[0].joueurs[1] = new IAJoueur(pseudo2.toStdString(), false);
    } else if (ia1) {
        historique_etats[0].joueurs[0] = new IAJoueur(pseudo1.toStdString(), true);
        historique_etats[0].joueurs[1] = new Joueur(pseudo2.toStdString(), false);
    } else if (ia2) {
        historique_etats[0].joueurs[0] = new Joueur(pseudo1.toStdString(), true);
        historique_etats[0].joueurs[1] = new IAJoueur(pseudo2.toStdString(), false);
    } else {
        historique_etats[0].joueurs[0] = new Joueur(pseudo1.toStdString(), true);
        historique_etats[0].joueurs[1] = new Joueur(pseudo2.toStdString(), false);
    ...
    // Ajouter les pièces des joueurs à la réserve
    for (Joueur* joueur : historique_etats[0].joueurs) {
        for (auto* piece : joueur->getPieces()) {
            historique_etats[0].plateau.addPieceReserve(*piece);
        }
    ...
    // Initialisation des états futurs
    for (int i = 1; i < 4; ++i) {
        historique_etats[i] = historique_etats[0];
    }
}
